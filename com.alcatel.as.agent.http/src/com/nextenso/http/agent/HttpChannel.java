 ...
 * channel.
 */
public class HttpChannel implements Runnable, AsyncProxyletManager.ProxyletResumer, PushletOutputStreamFactory, Socket {
	/*************************************************
	 * Public methods
	 *************************************************/

	public HttpChannel(int clientSockId, long sessionId, MuxConnection connection, Utils utils) {
		_utils = utils;
	 ...
	 * @param remoteIp
	 */
	public HttpChannel(int clientSockId, long sessionId, MuxConnection connection, Utils utils, String remoteIp) {
		this(clientSockId, sessionId, connection, utils);
		this.remoteIp = remoteIp;
	}

	public static HttpChannel getChannel(MuxConnection connection, int clientSockId, long sessionId, boolean create,
			Utils utils) {
		SocketManager mgr = connection.getSocketManager();
		HttpChannel channel = (HttpChannel) mgr.getSocket(Socket.TYPE_TCP, clientSockId);
		if (channel == null && create) {
			channel = new HttpChannel(clientSockId, sessionId, connection, utils);
			mgr.addSocket(channel);
		}
		return channel;
	}

	public static void closeSession(MuxConnection connection, Client client) {
		Enumeration<?> channels = connection.getSocketManager().getSockets(Socket.TYPE_TCP);
		while (channels.hasMoreElements()) {
			HttpChannel channel = (HttpChannel) channels.nextElement();
			if (channel.getClient() == client) {
				channel.sendSessionClose();
				break;
			}
		}
	}
	...
	// Returns the Client which is currently using this Http Channel.
	...
	public synchronized void storeRequestData(byte[] buf, int off, int len) {
		fillsRequestInput(buf, off, len);
	}

	public synchronized void handleResponse(byte[] buf, int off, int len) {
		client.accessed();
		fillsResponseInput(buf, off, len);
		handleResponse();
	}

	public synchronized void handleRespWentThrough() {
		client.accessed();
		respPassed = true;
		if (busy)
			return;
	...
	public synchronized void handleClientClose() {
	...
	public synchronized void handleServerClose() {
		serverClosed = true;
		if (busy)
			return;
		serverClose(true);
	...
	public boolean sendMuxData(MuxHeader hdr, boolean copy, ByteBuffer... buffers) {
		...
		return connection.sendMuxData(hdr, copy, buffers);
	}
	...
	protected void handleException(Throwable t, boolean pre) {
		...
		if (_logger.isEnabledFor(Level.ERROR)) {
			...
			text.append("(clid/sockId: ").append(Long.toHexString(sessionId)).append("/").append(clientSockId)
					.append("):");
			if (t instanceof ProxyletEngineException) {
				ProxyletEngineException exc = (ProxyletEngineException) t;
				text.append(" Proxylet: ").append(exc.getProxylet().getProxyletInfo());
				switch (exc.getType()) {
	...
	protected void handleNextHop() throws IOException {
		// Prepare the request: Handles the next hop
		if (request.nextHopModified()) {
			String nextHop = request.getNextHop();
			if (!nextHop.equals(HttpRequest.NEXT_HOP_DEFAULT)) {
				if (nextHop.equals(HttpRequest.NEXT_HOP_DIRECT))
					nextHop = Utils.SET_NEXT_HOP_DIRECT;
				else
					nextHop = Utils.SET_NEXT_HOP + nextHop;
				byte[] b = Utils.getBytes(nextHop, "ASCII");
				MuxHeaderV0 header = new MuxHeaderV0();
				header.set(sessionId, clientSockId, Utils.CONFIGURE_CONNECTION | Utils.PRE_FILTER_FLAGS);
				sendMuxData(header, false, ByteBuffer.wrap(b, 0, b.length));
			}
		}
	}
	...
	protected void requestDone() {
		...
		if (client.requestDone(activeChannel)) {
			// Send session close to the stack
			sendSessionClose();
		}
		...
		if (policy.getPolicy() == Policy.COOKIE) {
			HttpCookie cookie = response.getCookie(policy.getName());
			if (cookie != null) {
	...
	public void sendSessionClose() {
		if (client != null && client.isKeepAlive()) {
			if (_logger.isDebugEnabled()) {
				_logger.debug("Sending Session Close to http stack ...");
			}
			MuxHeaderV0 header = new MuxHeaderV0();
			header.set(sessionId, 0 /* indicates that clid is invalid */, Utils.CLOSED | Utils.POST_FILTER_FLAGS);
			_utils.getAgent().sendClose(connection, header);
		}
	}
	...
	public void sendClose(boolean pre, boolean close) {
		...
		int flags = (pre) ? Utils.PRE_FILTER_FLAGS : Utils.POST_FILTER_FLAGS;
		MuxHeaderV0 header = new MuxHeaderV0();
		header.set(sessionId, clientSockId, Utils.CLOSED | flags);
		_utils.getAgent().sendClose(connection, header);
	}
	...
	public synchronized void handleRequest() {
		...
		if (reqNeedsThread) { // a blocking stream proxylet(not currently running) needs a new
			...
			busy = true;
			try {
				Utils.getThreadPool().execute(this);
			} catch (Exception e) {
	...
	/**
	 * Retrieve the sessionId from the HTTP header, if any
	 */
	public synchronized void retrieveSessionIdFromHeader() {
		HttpRequestFacade req = new HttpRequestFacade();
		try {
			int result = clientSocket.readRequestHeaders(req);
			switch (result) {
			case HttpParser.READING_BODY:
			case HttpParser.PARSED:
				String sessionId = null;
				HttpCookie cookie = req.getCookie(SessionPolicy.COOKIE_SESSID);
				if (cookie != null) {
				...
				if (sessionId == null) {
					Object objSessionId = req.getURL().getParameterValue(SessionPolicy.PARAM_SESSID);
					if ((objSessionId != null) && (objSessionId instanceof String)) {
						sessionId = (String) objSessionId;
					}
				}
			...
			default:
				if (_logger.isDebugEnabled()) {
					_logger.debug("Incomplete parsing of the header. Status=" + result);
				}
			}
	...
	private void handleNewRequest(boolean ignoreMayBlock) {
		try {
			...
			if (!client.init(request, remoteIp))
				throw new IllegalStateException("Request refused by the Proxylet Engine");
			// If we have parsed an https CONNECT, keep waiting for more body.
			if (full) {
	...
	private void handleBodyRequest() {
		...
		try {
			...
			if (lastPart) {
				if (state == ST_RESPOND_REQ) { // a request proxylet has responded to the request
					respond(); // we can now send the response since we have received the last chunk.
				}
	...
	private void respond() throws IOException {
		// Send the response to the client and unregister our Channel from active
		// channels
		// When we send the response to the client, this one may send another request to
		// our
		// agent.
		// So, if we are currently running in the thread pool, we might receive another
		// request
		// just before invoking "requestDone".
		// To avoid such race condition, we synchronize of the channels hashtable
		// (remember that
		// the HttpChannel.getChannel is invoked by the Agent, once a packet is received
		// ...)

		response.writeTo(serverSocket.getOutputStream(), true);
		requestDone();
		// TODO really need to be synchronized ???
//    Hashtable<Integer, HttpChannel> channels = Utils.getChannels(connection);
//    synchronized (channels) // avoid the client to send another request before we call requestDone ...
//    {
//    }
	}
	...
	protected synchronized void handleResponse() {
	...
	private void handleNewResponse(boolean ignoreMayBlock) {
		try {
			...
			_logger.debug("H2 response is FULL: " + full);
	...
	private synchronized boolean drainHTTP2ResponseBodyBuffer() { // returns true if response is fully parsed
		...
		if (h2BodyResponseBuffer == null) {
			return false;
		}

		ByteBuffer b;
		while ((b = h2BodyResponseBuffer.poll()) != null) {
...
//    return (left < BODY_CHUNK_SIZE_DEF);
	...
	private synchronized boolean checkThreadExit() {
		if (isSuspended()) {
		...
		if (earlyResponse != EARLY_RESP_NONE) {
			...
			if (state < ST_READY_RESP) {
				...
				if (isH2) {
					...
					if (h2BodyResponseBuffer.isEmpty() && !isH2ResponseFull) {
						busy = false;
						return true;
					}
	...
	/**
	 * Called when the proxylet resumes. The caller thread can be the http queud, or
	 * the IO threadpool
	 */
	public synchronized void resumeProxylet(ProxyletData msg, int status) {
		if (!isSuspended()) {
	...
	// Instance variables
	protected int state;
	protected volatile boolean busy; // If true: either a thread is alive, or we are suspended.
	...
	protected HttpSocket clientSocket;

	protected HttpSocket serverSocket;
	private int mayBlockCode, resumeStatus;
	protected boolean reqNeedsThread, respNeedsThread;
	private volatile boolean clientClosed, serverClosed, closedPerformed, redirected, respPassed;
	...
	protected volatile boolean isH2 = false;

	protected volatile Queue<ByteBuffer> h2BodyResponseBuffer = null;
	protected volatile boolean isH2ResponseFull = false;
	...
	protected volatile boolean websocket;
		...
		private void doFlushDirect(byte[] data, int off, int len, boolean doClose) {
			try {
				...
				if (doClose) {
					...
					_utils.getAgent().sendClose(connection, hdr);
				}
			}
	...
	public String toString() {
		return this.getClass().getSimpleName() + " [sockId=" + clientSockId + ", clid=" + Long.toHexString(sessionId)
