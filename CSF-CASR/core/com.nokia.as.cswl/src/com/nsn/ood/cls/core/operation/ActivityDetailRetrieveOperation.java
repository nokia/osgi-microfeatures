 ...
 * 
 */
@Component(provides = ActivityDetailRetrieveOperation.class)
@Loggable
public class ActivityDetailRetrieveOperation extends AbstractRetrieveOperation<ActivityDetail, QueryActivityDetails> {
	@ServiceDependency
	private ActivityCreator creator;
	@ServiceDependency(filter = "(name=activityDetail)")
	...
	@Override
	protected QueryActivityDetails createQuery(final Conditions conditions) throws ConditionProcessingException {
		return new QueryActivityDetails(conditions, this.mapper, this.creator);
	}
