...
final class SSLSessionImpl extends ExtendedSSLSession {
    ...
    // -- Export
    private SecretKey exportKey = null;
    public SecretKey getExportKey() {
        if (exportKey == null) {
            for (SSLSessionImpl x : childSessions) {
                if (x.exportKey != null)
                    return x.exportKey;
            }
        }
    ...
    public void setExportKey(SecretKey exportKey) {
        TraceUtils.here("setExportKey: "+TraceUtils.session(this));
    ...
    SSLSessionImpl(HandshakeContext hc,
        ...
        if (protocolVersion == ProtocolVersion.TLS13) {
            this.exportKey = hc.getExportSecret();
            if (exportKey != null) {
                // This behavior is specific to CASR (using SSLEngine)
                hc.conContext.conSession.setExportKey(exportKey);
            } else {
                if (hc instanceof ClientHandshakeContext) {
                    hc.exportToSession = this;
                }
                if (hc.conContext.conSession.exportKey != null) {
                    this.exportKey = hc.conContext.conSession.exportKey;
                }
            }
        } else {
            if (hc.conContext.conSession.exportKey != null) {
        ...
        if (trace)
        TraceUtils.here(
                "invocation of constructor for SSLSessionImpl"
                        +" from hc: "   + TraceUtils.session(hc.handshakeSession)
                        +" to this: "+ TraceUtils.session(this)
        );
        // -- Log
...
//        // -- Export
        ...
        TraceUtils.here(
                "invocation of constructor for SSLSessionImpl"
                         +" from: "   + TraceUtils.session(baseSession)
                         +" to this: "+ TraceUtils.session(this)
                );
        // -- Log
    ...
    void addChild(SSLSessionImpl session) {
        ...
        TraceUtils.here(
                "addCchild on"
                        +" from: "   + TraceUtils.session(session)
                        +" to this: "+ TraceUtils.session(this)
        );
        // -- Log
    }
    ...
    public String toString() {
        return "Session(" + creationTime + "|" + getCipherSuite() + "@" + hashCode() + ")";
    }
}
