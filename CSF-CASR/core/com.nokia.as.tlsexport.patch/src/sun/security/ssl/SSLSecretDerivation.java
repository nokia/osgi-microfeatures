...
final class SSLSecretDerivation implements SSLKeyDerivation {
    ...
    // -- Export
    public SecretKey deriveKey_(String algorithm,
            AlgorithmParameterSpec params) throws IOException {
        return deriveKey_(algorithm,params, hashAlg.hashLength);
    }
    // -- Export
    public SecretKey deriveKey_(String algorithm,
            AlgorithmParameterSpec params, int length) throws IOException {
        SecretSchedule ks = SecretSchedule.valueOf(algorithm);
        try {
            byte[] expandContext;
                if (hashAlg == HashAlg.H_SHA256) {
