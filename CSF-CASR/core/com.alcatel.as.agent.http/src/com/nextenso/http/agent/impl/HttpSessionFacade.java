...
import com.alcatel_lucent.ha.services.FlattableVisitor;
...
import com.nextenso.http.agent.ext.HttpSessionExt;
import com.nextenso.proxylet.ProxyletContext;

public class HttpSessionFacade implements HttpSessionExt, Flattable {
  ...
  // Constructor called by some web pushlet HA classes, we we assume that we are running
  // in a single agent JVM (elasticity=1).
  public HttpSessionFacade() {
    this(Utils.getDefaultUtils());
  }
  ...
  public String changeSessionId() {
    return sessionMngr.changeSessionId();
  }
  ...
  public void read(FlattableVisitor visitor, Map<String, Object> map) throws IllegalArgumentException,
      IllegalAccessException {
    
    visitor.visit(this, map);
    sessionCookieSet();
    updateAccessedTime();
    updateAccessedTimeOnReq();
    tmpmaxinterval = ((Integer) map.get(_k + Flattable.KEYSEP + "M")).intValue();
  ...
  public boolean write(Map<String, Object> map, FlattableVisitor visitor) throws IllegalArgumentException,
      IllegalAccessException {
    visitor.visit(map, this);
    map.put(_k + Flattable.KEYSEP + "M", getMaxInactiveInterval());
    diff(true);
    return true;
  }
