...
public class Ensure {
    ...
    public synchronized void step(int nr) {
        ...
        try {
        	...
        	m_throwable = e;
			notifyAll(); // wakeup the waitForStep method
        	throw e;
        }
        if (DEBUG) {
    ...
    private void checkErrorThrown() {
		if (m_throwable instanceof RuntimeException) {
			throw (RuntimeException) m_throwable;
		} else if (m_throwable != null) {
			throw new RuntimeException("an exception has been caught", m_throwable);
		}		
	}
     ...
     * takes more than <code>10</code> seconds. If you invoke wait on a thread,
     * you are effectively assuming some other thread will invoke the <code>step(nr)</code>
     * method.
     * 
     * @param nr the step to wait for
     */
    public synchronized void waitForStep(int nr) {
    	waitForStep(nr, 10);
    }
     ...
     * 
     * @param nr the step to wait for
     * @param timeoutSec the number of seconds to wait
     */
    public synchronized void waitForStep(int nr, int timeoutSec) {
    	checkErrorThrown();
    	timeoutSec *= 1000; // convert to millis;
        final int initialTimeout = timeoutSec;
        if (DEBUG) {
            String info = getLineInfo(3);
            STREAM.println("[Ensure " + INSTANCE + "] waiting for step " + nr + " [" + currentThread() + "] " + info);
        }
        while (step < nr && timeoutSec > 0) {
            try {
                wait(RESOLUTION);
                timeoutSec -= RESOLUTION;
                checkErrorThrown();
            }
