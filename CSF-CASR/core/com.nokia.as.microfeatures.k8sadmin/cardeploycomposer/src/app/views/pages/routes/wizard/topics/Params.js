...
class Params extends Component {
  ...
  componentWillMount() {
    console.log("Params componentWillMount props", this.props);
  ...
  componentWillUnmount() {
    console.log("Params componentWillUnmount");
    // Save the warnings before navigating to other pages
    // const { paramNamesError, ...warnings } = this.globalErrors;
  ...
  //
  // Save the Param list in Redux and its complete status
  //
  ...
  validate = () => {
    ...
    if (paramNames.length > uniqParamNames.length) {
      this.globalErrors.paramNamesError =
        "Some parameters names are identicals";
      validation.errors.push(this.globalErrors.paramNamesError);
    } else {
      this.globalErrors.paramNamesError = "";
    }

    console.log(
      "<Params validate() return validation",
      this.errorsByIdx,
      this.globalErrors,
      validation
    );
  ...
  render() {
    console.log("Params render() props", this.props, this.state);
    const { paramNamesError } = this.globalErrors;

    return (
      <div>
        <div>{this.renderItemList(this.props.elements, this.renderItem)}</div>
        <div className={"colorErrorList"}>
          {generateFormError([paramNamesError])}
        </div>
        <div>
          <Button
            id="addParamBtn"
            text="ADD ROUTE PARAMETER"
            isCallToAction
            disabled={false}
          ...
          <Label
            id="paramleg"
            text="Route parameters [optional]"
          />
          </div>
        </div>
      </div>
