...
final class CipherCore {
    ...
    void setMode(String mode) throws NoSuchAlgorithmException {
        ...
        if (modeUpperCase.equals("CBC")) {
            ...
            if (blockSize != 16) {
                throw new NoSuchAlgorithmException
                    ("CCM mode can only be used for AES cipher");
            }
            cipherMode = CCM_MODE;
            cipher = new CounterWithCbcMacMode(rawImpl);
            padding = null;
    ...
    private int getOutputSizeByOperation(int inputLen, boolean isDoFinal) {
        ...
        case CCM_MODE:
            if (isDoFinal) {
                int tagLen = ((CounterWithCbcMacMode) cipher).getTagLen();
                if (!decrypting) {
                    totalLen = Math.addExact(totalLen, tagLen);
                } else {
                    totalLen -= tagLen;
    ...
    AlgorithmParameters getParameters(String algName) {
        ...
        if (iv == null) {
            ...
            } if (cipherMode == CCM_MODE) {
                iv = new byte[CounterWithCbcMacMode.DEFAULT_IV_LEN];
            } else {
        ...
        if (cipherMode == GCM_MODE) {
            ...
            algName = "CCM";
            spec = new CCMParameterSpec
                    (((CounterWithCbcMacMode) cipher).getTagLen()*8, iv);
        } else {
    ...
    void init(int opmode, Key key, AlgorithmParameterSpec params,
        ...
        if (params != null) {
            if (cipherMode == GCM_MODE) {
                ...
                if (params instanceof CCMParameterSpec) {
                    // FIXME: Check spec for this
                    tagLen = ((CCMParameterSpec)params).getTLen();
                    ...
                    tagLen = tagLen >> 3;
                    ivBytes = ((CCMParameterSpec)params).getIV();
                } else {
                    throw new InvalidAlgorithmParameterException
                            ("Unsupported parameter: " + params);
                }
        ...
        if (cipherMode == ECB_MODE) {
            ...
            if (cipherMode == GCM_MODE) {
                ...
                ivBytes = new byte[CounterWithCbcMacMode.DEFAULT_IV_LEN];
            } else {
        ...
        if (cipherMode == GCM_MODE) {
            ...
            if(tagLen == -1) {
                tagLen = CounterWithCbcMacMode.DEFAULT_TAG_LEN;
            }
            if (decrypting) {
                minBytes = tagLen;
            } else {
                // check key+iv for encryption in CCM mode
                requireReinit =
                        Arrays.equals(ivBytes, lastEncIv) &&
                                MessageDigest.isEqual(keyBytes, lastEncKey);
                if (requireReinit) {
            ...
            ((CounterWithCbcMacMode) cipher).init
                    (decrypting, algorithm, keyBytes, ivBytes, tagLen);
        } else {
        ...
        if (params != null) {
            try {
                if (cipherMode == GCM_MODE) {
                    ...
                    paramType = "CCM";
                    spec = params.getParameterSpec(CCMParameterSpec.class);
                } else {
    ...
    byte[] update(byte[] input, int inputOffset, int inputLen) {
        if (requireReinit) {
            throw new IllegalStateException
                ("Must use either different key or iv for CCM/GCM encryption");
        }
    ...
    int doFinal(byte[] input, int inputOffset, int inputLen, byte[] output,
        ...
        if (decrypting) {
            ...
            try {
                ...
                requireReinit = (cipherMode == GCM_MODE || cipherMode == CCM_MODE);
            }
        }
    ...
    private int finalNoPadding(byte[] in, int inOfs, byte[] out, int outOfs,
        ...
        if ((cipherMode != GCM_MODE && cipherMode != CCM_MODE) && (in == null || len == 0)) {
            return 0;
        }
