...
public abstract class HttpMessageFacade extends ProxyletDataImpl implements HttpObject, HttpHeaders,
    HttpBody, HttpMessage, AsynchronousEventListener, HttpHandler, Recyclable {
  /**
   ...
   */
  protected HttpMessageFacade(ByteBuffer content) {
  ...
  public void setSession(HttpSessionFacade session) {
  ...
  public HttpSessionFacade getSession(boolean create) {
	  if (create) {
		  synchronized (this) { //can be called from any thread
			  if (session == null) {
				  session = HttpSessionManager.instance().getSession (true, this); 
			  }
		  }
	  }
	  return session;
  ...
  private void toString(StringBuilder buf, boolean withBody, boolean noBinary) {
    ...
    toStringCapitalizeHeader(buf, keys);
    toStringCookies(buf);
    ...
    if (withBody && content.size() > 0) {
      if (noBinary) {
        ...
        if (contentType != null && contentType.regionMatches(true, 0, "multipart/", 0, "multipart/".length())) {
          Multipart mp = new Multipart(getHeaders().getHeader(HttpUtils.CONTENT_TYPE), content.toByteArray(false), 0,
              content.size());
...
private void toStringCapitalizeHeader(StringBuilder buf, Enumeration<?> keys) {
  ...
  public void recycled() {
	  content.init();
	  protocol = HttpUtils.HTTP_11;
	  hdrs.clear();
	  cookies.clear();
	  session = null;
	  chunked = false;
	  proxylets.clear();
	  connectionHeaderModified = false;
	  super.recycled();
  }
  ...
  protected Vector<HttpCookie> cookies = new Vector<HttpCookie>(2); // we use a Vector to be able to use the method "elements ()"
  protected HttpSessionFacade session;
