 ...
 */
public class TestServer implements DiameterRequestProxylet, TestServerMBean {
    
	private final static Logger _log = Logger.getLogger(TestServer.class);
	...
	public void init(ProxyletConfig config) throws ProxyletException {
		...
		_log.warn("***************** TestServer : init : myparam=" + value + ", instance=" + this);
	}
	...
	public int doRequest(DiameterRequest request) {
		...
		_log.warn("***************** TestServer : doRequest : " + request + ", count=" + _count);
		...
		// yet we dont plan to have response proxylet inthe response chain
