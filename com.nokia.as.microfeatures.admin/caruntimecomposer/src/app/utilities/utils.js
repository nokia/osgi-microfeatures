...
const decodeId = (id, s1 = 'name', s2 = 'version', tab = id.split('@', 2)) => { let x = {}; x[s1] = tab[0]; x[s2] = tab[1]; return x }

export const encodeFeatureId = feature => generateId(feature.name, feature.version)
...
function createRequestTypes(base) {
    return [msgTypes.REQUEST, msgTypes.SUCCESS, msgTypes.FAILURE].reduce((acc, type) => {
...
//    console.log("fetchEntity", response, error, id);
    if (response)
        yield put(entity.success(id, response,params))
    else
        yield put(entity.failure(id, error,params))
}
...
// Assembly tools
const LDAP_REGEXP = /.*\(.*feature=(.*)\)\(version=(.*)\)\)\).*/
const getFeatureIdsFromLdapFilters = (ldapfilters) => {
    const ids = []
    for (let i = 0; i < ldapfilters.length; i++) {
        let id = getFeatureIdFromLdapFilter(ldapfilters[i])
        if (id)
            ids.push(id)
    }
    return ids
...
const getFeatureIdFromLdapFilter = (ldapFilter) => {
    // Reset global regexp index to parse properly
    LDAP_REGEXP.lastIndex = 0
    const match = LDAP_REGEXP.exec(ldapFilter)
    if (!match || match.length < 3) {
        console.warn("getfeatureIdFromLdapFilter no matching", ldapFilter, match)
        return undefined;
    }
    return generateId(match[1], match[2])
}
...
const parseBundles = function (xml) {
    const obr = $(xml)
    const obrBundles = {};
    obr.find("resource").each(
        function () {
            try {

                var identity = $(this).find("capability[namespace='osgi.identity']").first();
                var symbolicname = identity.find("attribute[name='osgi.identity']").attr("value");
