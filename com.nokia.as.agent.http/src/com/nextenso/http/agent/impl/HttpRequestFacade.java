...
public class HttpRequestFacade extends HttpMessageFacade implements HttpRequest, HttpRequestProlog,
    HttpRequestHandler, Recyclable {
  // the initial size for content
  public static int CONTENT_INIT_SIZE = 0; // 128 for request
  ...
  public String getRemoteAddr() {
	  String headerName = com.nextenso.http.agent.Utils.INSTANCE.getClipHeaderName();
	  String headerVal = (headerName != null) ? getHeader(headerName) : null;
	  if (headerVal != null) {
	      return headerVal;  // From header 
	  } else {
		  return remoteAddr; // From tcpSocketConnect
	  }
  }
  ...
  public String getRemoteIp() { // from tcpSocketConnected
	  return remoteAddr; // From tcpSocketConnect
  }
  ...
  public void setHttpRequestUri(String uri, boolean relativeUrl) throws java.net.MalformedURLException {
    ...
    if (method.equals(HttpUtils.METHOD_CONNECT)) {
    	if (! relativeUrl) {
    		url = new HttpURL("http://" + uri);
    		// the addHttpHeader will be called and will possibly override the url using proper scheme/auth
    	}
    } else {
      if (relativeUrl) {
  ...
  public void recycled() {
	  ...
	  id = -1;
	  url = null;
	  nextHop = NEXT_HOP_DEFAULT;
	  nextHopModified = false;
	  nextProxy = null;
	  nextServer = null;
	  proxyMode = true;
	  remoteAddr = null;
  ...
  private String remoteAddr; // from TcpSocketConnected
