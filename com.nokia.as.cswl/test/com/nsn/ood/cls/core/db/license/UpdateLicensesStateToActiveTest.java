 ...
 * 
 */
public class UpdateLicensesStateToActiveTest {
	private static final DateTime DATE = new DateTime(2015, 10, 28, 17, 1);

	@Test
	public void testSql() throws Exception {
		assertEquals("update cls.licenses set state = '" + LicenseState.ACTIVE.toString() + "'"//
				+ " where state = '" + LicenseState.INACTIVE.toString() + "' and startdate <= ?",
				new UpdateLicensesStateToActive(null, null).sql());
	}
	...
	public void testPrepare() throws Exception {
		...
		replayAll();
		new UpdateLicensesStateToActive(DATE, converterMock).prepare(statementMock);
		verifyAll();
	}
}
